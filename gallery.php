...
<!-- Filter Sidebar Styles (matching anni_gal.php) -->
<style>
...
/* Active Service - Green Highlight */
.service-item.active-service {
	background-color: #A4B82C;
	color: white;
	font-weight: 700;
	border-left: 5px solid #8B9621;
}
...
.category-title {
	...
	padding-bottom: 10px;
	border-bottom: 3px solid #A4B82C;
}

/* Venue Card Styles (matching anni_gal.php) */
.venue-card {
	background: white;
	border-radius: 12px;
	overflow: hidden;
...
<div class="gallery-container-wrapper">
	...
	<aside class="filter-sidebar">
		...
		<div class="filter-content">
			<form method="GET" action="gallery.php" id="filterForm">
				<!-- Location Filter -->
		...
		<div class="services-content">
			<a href="gallery.php" class="service-item-link">
				<div class="service-item active-service">Wedding</div>
			</a>
			<a href="bday_gal.php" class="service-item-link">
				<div class="service-item">Birthday Party</div>
			</a>
			<a href="anni_gal.php" class="service-item-link">
				<div class="service-item">Anniversary</div>
			</a>
			<a href="other_gal.php" class="service-item-link">
				<div class="service-item">Entertainment</div>
			</a>
		</div>

		<div class="filter-content">
			<a href="gallery.php" class="btn-reset-filters">Reset Filters</a>
		</div>
	</aside>
	...
	<div class="gallery-main-content">
		<div class="container" style="max-width: 100%; width: 100%; padding: 0;">
			...
			<?php
				...
				// BUILD SINGLE QUERY FOR ALL WEDDING VENUES
				$qry_all = "SELECT * FROM wedding WHERE 1=1";
			...
			<!-- ALL WEDDING VENUES SECTION -->
			<div class="gallery-grids-top">
				<h3 class="category-title">Wedding Venues & Services (<?php echo $count_all; ?> Results)</h3>

				<div class="gallery-grids" style="display: flex; flex-wrap: wrap; gap: 20px;">
				<?php
					if($count_all == 0){
						echo "<div style='width: 100%; padding: 40px; text-align: center;'>
							<div style='background: white; padding: 40px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);'>
								<h3 style='color: #8B2E2E; margin-bottom: 10px;'>No venues found</h3>
								<p style='color: #666;'>Try adjusting your filters to see more results.</p>
								<a href='gallery.php' style='display: inline-block; margin-top: 20px; padding: 12px 30px; background: #8B2E2E; color: white; text-decoration: none; border-radius: 6px;'>Reset Filters</a>
							</div>
						</div>";
					}
				...
				<div style="width: 100%; margin-bottom: 30px;">
					<div class="venue-card">
						...
						<div class="venue-image">
							<a class="example-image-link" href="images/<?php echo $row['img']; ?>" data-lightbox="wedding-set">
						...
						<div class="venue-details">
							<div>
								...
								<div class="venue-rating">
									...
									echo " <small>(" . number_format($rating, 1) . ")</small>";
									?>
								</div>
								...
								<div class="venue-info">
									<p>
										...
										NPR <?php echo isset($row['price']) ? number_format($row['price']) : '0'; ?>
									</p>
									<p>
										<strong>Seat Capacity:</strong>
										<?php echo isset($row['capacity']) ? number_format($row['capacity']) : '0'; ?> people
									</p>
									<p>
										<strong>Venue Type:</strong>
										<?php echo isset($row['venue_type']) ? $row['venue_type'] : 'N/A'; ?>
									</p>
									<p>
										<strong>Space Preference:</strong>
										<?php echo isset($row['space_preference']) ? $row['space_preference'] : 'N/A'; ?>
									</p>
									<p>
										<strong>Service Location:</strong>
										<?php echo isset($row['location']) ? $row['location'] : 'N/A'; ?>
									</p>
								</div>
							</div>
							...
							<!-- Details Button -->
							<div>
								<a href="book_wed.php?id=<?php echo $row['id']; ?>" class="details-btn">
									DETAILS
								</a>
							</div>
						</div>
